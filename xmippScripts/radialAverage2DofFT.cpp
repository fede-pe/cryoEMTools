 ...
 *
 * ESTO ES UNA ÑAPA PARA PODER LEER MODULOS DE FT QUE TIENEN EL ORIGEN CAMBIADO
 * This program calculate the fourier transform and the radial 2D average of 
...
int main(int argc, char **argv)
{
	...
	for(size_t k=1; k<xSize; ++k){
		FFT_IDX2DIGFREQ(k,(XSIZE(img())-1)*2, u);
	...
	for(size_t i=0; i<YSIZE(fftImg); ++i)
	{
		...
		for(size_t j=0; j<XSIZE(fftImg); ++j)
		{
			...
			idx = (int) round(ux * (XSIZE(img())-1)*2);
	...
	{
		double value_mod  = DIRECT_MULTIDIM_ELEM(fftImg,n);
